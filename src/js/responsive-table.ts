...
class ResponsiveTable {
  ...
  constructor(elem: HTMLTableElement) {
    this.elem = elem;
    this.heads = $$('th:not(th:only-of-type)', elem) as HTMLTableHeaderCellElement[];
  ...
  init() {
    ...
    this.rows.forEach((row: HTMLTableRowElement, rowIndex: number) => {
      ...
      const rowLabel = this.heads.length !== 0 ? (this.rowHeads[rowIndex - 1] ? this.rowHeads[rowIndex - 1].innerText : '') : (this.rowHeads[rowIndex] ? this.rowHeads[rowIndex].innerText : '');

      cells.forEach((cell: HTMLTableCellElement, index: number) => {
        const label = this.heads[index] ? (rowLabel ? `${this.heads[index + 1].innerText}, ${rowLabel}`: this.heads[index].innerText) : (rowLabel ? rowLabel : '');
