...
function renderPercentBarChart(el: any, config: any) {
...
class MiddChart {
  ...
  getBaseOptions() {
    ...
    if (title !== '') {
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'title' does not exist on type '{ maintai... Remove this comment to see the full error message
      options.title = {
        display: true,
        text: title,
        fontSize: 14,
    ...
    if (isAxisChart) {
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'scales' does not exist on type '{ mainta... Remove this comment to see the full error message
      options.scales = {
        xAxes: [
          {
            scaleLabel: {
  ...
  draw() {
    ...
    this.chart = new Chart(this.canvas, {
      type,
      data: {
        datasets: datasets.map((d: any, i: any) => {
